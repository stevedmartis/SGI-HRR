...
class PedidoEditForm(forms.ModelForm):
    cantidad       = forms.IntegerField(label='Cantidad:', widget=forms.TextInput(attrs={'type':'number', 'size':'10'}))
...
class PedAdminEditForm(forms.ModelForm):
    cantidad_update       = forms.IntegerField(label='Cantidad:', widget=forms.TextInput(attrs={'type':'number', 'size':'10'}))
