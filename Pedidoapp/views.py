...
class ReporteTotalPDF(View): 
    ...
    def tabla1(self,pdf,y):
        ...
        #Creamos una lista de tuplas que van a contener a las personas
        detalles = [(art.cod_experto, art.nombre, art.stock, art.info_bodega, art.total_pedido) for art in Articulo.objects.filter(cod_experto__range=["AA-0001", "DRE-0054"]).filter(info_bodega=1).filter(total_pedido__gt=0).order_by('cod_experto')]
        #Establecemos el tamaño de cada una de las columnas de la tabla
