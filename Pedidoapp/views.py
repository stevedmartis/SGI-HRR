...
class ReportePedidosPDF(View): 
    ...
    def tabla(self,pdf,y, id_especialidad):
        ...
        encabezados = ('Especialidad', 'Codigo Experto', 'Nombre Articulo', 'Cantidad', 'Cantidad Moficada', 'Estado')
        #Creamos una lista de tuplas que van a contener a las personas
        detalles = [(pedido.especialidad.nombre, pedido.articulo.cod_experto, pedido.articulo.nombre, pedido.cantidad, pedido.cantidad_update, pedido.estado) for pedido in Pedido.objects.filter(especialidad=especialidad)]
        #Establecemos el tamaño de cada una de las columnas de la tabla
        detalle_orden = Table([encabezados] + detalles, colWidths=[5 * cm, 5 * cm, 10 * cm, 2 * cm , 3 * cm, 2 * cm])
