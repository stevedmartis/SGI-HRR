...
class ReportePedidosPDF(View): 
    ...
    def cabecera(self,pdf, id_especialidad):
        ...
        pdf.drawString(1000, 3250, u"PEDIDO POR ESPECIALIDAD")
        pdf.setFont("Helvetica", 16)
        pdf.drawString(1050, 3000, u"FECHA: " + str(datetime.date.today()))
        pdf.setFont("Helvetica", 16)
        pdf.drawString(100, 3000, u"Solicitado por: " + str(especialidad.encargado.nombre))
    ...
    def tabla(self,pdf,y, id_especialidad):
        ...
        if count <=30:
              ...
              detalle_orden.drawOn(pdf, 440, 1700)
        elif count >30 and count <=50:
              #Definimos la coordenada donde se dibujará la tabla
              detalle_orden.drawOn(pdf, 440, 1250)
        elif count >50 and count <=70:
              #Definimos la coordenada donde se dibujará la tabla
              detalle_orden.drawOn(pdf, 440, 1000)
        elif count >70 and count <=90:
              #Definimos la coordenada donde se dibujará la tabla
              detalle_orden.drawOn(pdf, 440, 580)
